...
var GInfoWindow = function (pParent, pEarth, callback, title, content, locationx, locationy) {
    ...
    function select(i) {
		...
		var labelposition = i * tLabelWidth;
		//狀態估測 Soon 1100725
		if(i == 4)
			labelposition = 2 * tLabelWidth;
        $("#" + that.ScrollbarID).css("transform", "translate3d(" + labelposition + "px, 0px, 0px)");
        switchDiv(i);
    }
    /**
     * 切換頁面
    ...
    function switchDiv(i) {
        ...
        switch (i) {
            //0:設備資訊 1:擁有者 2:即時資料 3:Error顯示(不能換到其他頁) 4:狀態估測頁
            case 0:
                $("#tblBasicInfo").show();
                $("#tblOwnerInfo").hide();
	...
	//Soon 1100725 狀態估測頁籤調整
	var isFTUswitch = (content.RealtimeDataTable == null || content.RealtimeDataTable.length == 0) ? false : true;
	var isSEswitch = SEMarkers.ufidList.includes(content.iEquipmentSerial) ? true : false;
	if(isFTUswitch && isSEswitch)
	{
		...
		[
            ...
            { Text: "狀態估測", OnClick: function () { select(4); } }
		]
		);
	}
	else if (isFTUswitch)
	...
	*/
	...
	//狀態估測 Soon 1100725
	var tb4 = tr1.appendObject("table", { id: "tblSEResultInfo", class: "DisplayAttribute" });
    //SCADA定位紐樣式由ID來定
    ...
    AttachEvent(SCADAButton, "click", function () {
        // if (content.FeederID.length > 0) {
            ws.send("GISUI,11,TPC_GRID=" + Position_TPC + "," + $("#CURRENTUSERID").text());
        // }
    });
    //開始處理content
    if ((content.BasicInfo == undefined || content.BasicInfo.length == 0) && (content.OwnerInfo == undefined || content.OwnerInfo.length == 0)) {
		...
		if (isSEswitch)
		{
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			var index = SEMarkers.ufidList.indexOf(content.iEquipmentSerial);
			var seresult = estResults[index];
			var values = seresult.attributes.values;
            var feederid = values[1]; // 饋線編號
            var coordinate = values[2]; // 圖號坐標
            var loopid = values[3]; // 迴路別
			tr4.appendObject("td", { class: "FieldName" }).appendText("圖號坐標");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(coordinate);
			tr4.appendObject("td", { class: "FieldName" }).appendText("迴路別");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(loopid);
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			tr4.appendObject("td", { class: "FieldName" }).appendText("估測電壓R");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(values[4]);
			if(seresult.auto)
			{
				tr4.appendObject("td", { class: "FieldName" }).appendText("估測電壓R誤差");
				tr4.appendObject("td", { class: "FieldValue" }).appendText(values[10]+'('+values[13]+'%)');
			}
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			tr4.appendObject("td", { class: "FieldName" }).appendText("估測電壓S");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(values[5]);
			if(seresult.auto)
			{
				tr4.appendObject("td", { class: "FieldName" }).appendText("估測電壓S誤差");
				tr4.appendObject("td", { class: "FieldValue" }).appendText(values[11]+'('+values[14]+'%)');
			}
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			tr4.appendObject("td", { class: "FieldName" }).appendText("估測電壓T");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(values[6]);
			if(seresult.auto)
			{
				tr4.appendObject("td", { class: "FieldName" }).appendText("估測電壓T誤差");
				tr4.appendObject("td", { class: "FieldValue" }).appendText(values[12]+'('+values[15]+'%)');
			}
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			tr4.appendObject("td", { class: "FieldName" }).appendText("估測電流R");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(values[7]);
			if(seresult.auto)
			{
				tr4.appendObject("td", { class: "FieldName" }).appendText("估測電流R誤差");
				tr4.appendObject("td", { class: "FieldValue" }).appendText(values[16]+'('+values[19]+'%)');
			}
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			tr4.appendObject("td", { class: "FieldName" }).appendText("估測電流S");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(values[8]);
			if(seresult.auto)
			{
				tr4.appendObject("td", { class: "FieldName" }).appendText("估測電流S誤差");
				tr4.appendObject("td", { class: "FieldValue" }).appendText(values[17]+'('+values[20]+'%)');
			}
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			tr4.appendObject("td", { class: "FieldName" }).appendText("估測電流T");
            tr4.appendObject("td", { class: "FieldValue" }).appendText(values[9]);
			if(seresult.auto)
			{
				tr4.appendObject("td", { class: "FieldName" }).appendText("估測電流T誤差");
				tr4.appendObject("td", { class: "FieldValue" }).appendText(values[18]+'('+values[21]+'%)');
			}
			//緩衝
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
			var tr4 = tb4.appendObject("tr", null, { height: "22px" });
