...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
var GTopoResultDialog = function (pParent, pEarth, jsonobject, callback) {
    //基於繼承，這個是一定要先call的
    ...
    function Highlight(pm, restore, sec, color) {
        if (color == null)
            color = [1, 0, 0];
        ...
        return pm.Highlight(color, restore, sec);
    }
    function GetPlacemarkByUFID(ufid) {
        for (var i in Layers) {
            var layer = Layers[i];
            ...
            for (var j in tiles) {
                ...
                for (var k in pms) {
                    var pm = pms[k];
                    if (pm.GetFieldValueByIndex(2) == ufid)
                        return pm;
                }
            }
        }
    ...
    function ClickElement(x, y) {
        for (var i in elements) {
            if (x > elements[i].x && x < elements[i].x + eleW && y > elements[i].y && y < elements[i].y + eleH) {
                ...
                earth_.SetViewpoint(pt.X, pt.Y, pCam.Position.Z, pCam.Yaw, 0, false);

                var pm = GetPlacemarkByUFID(parseInt(i));
                if (pm)
                    Highlight(pm, true, 1);
            }
        }
    ...
    function DrawElement(dv, ori, canvas) {
        ...
        ctx.fillText(dv.ufid, ori.x + eleW / 2 + txtOffset, ori.y + eleH / 3 * 2 + 1);
        ...
        for (var i in dv.children) {
            ...
            //    alert("repeated ufid");
            elements[ufid] = { x: ori_child.x, y: ori_child.y, mapx: child.x, mapy: child.y };
    ...
    this.BodyCell.applyStyles({ position: "relative", overflow: "auto", width: "500px", height: "300px", maxWidth: "500px", maxHeight: "300px" });
    ...
    ctx.font = "11.5px Arial";
    elements = {};
    var start = { x: 0.5, y: 10.5, mapx: jsonobject.x, mapy: jsonobject.y };
    elements[jsonobject.ufid] = start;
...
var GFeederLineAnalysisDialog = function (pParent, pEarth, callback) {
    ...
    this.EquipClass_Properties = [
        {
            ...
            classname: "線路變壓器", tablename: "sxfmr", props: [
                { propname: "第一具變壓器容量", propfield: "sxfmrcapacity1" },
                { propname: "第二具變壓器容量", propfield: "sxfmrcapacity2" },
                { propname: "第三具變壓器容量", propfield: "sxfmrcapacity3" },
                {propname:"用戶數"}
            ]
        }, {
    ...
    function switchDiv(i) {
        ...
        if (i == 0) {
            WriteUserLog(7);
            that.getNode().style.height = "370px";
    ...
    function funcMouseUp(tEvent) {
        ...
        switch (_3DMouseUpDown.MouseType) {
            ...
            case ClickFor.PickTopology:
                ...
                var tid = pm.GetFieldValueByIndex(1);
    ...
    function StartAnalysis() {
        document.getElementById("btnFeederCalculation").disabled = true;
        ...
        $.get("../../FeederAnalysis/TopoByFeeder/" + code, null, function (str) {
            document.getElementById("btnFeederCalculation").disabled = false;
            ...
            TopologyGraphWindow = new GTopoResultDialog(pParent, pEarth, dv, callback);
    ...
    function CloseFeederAni() {
        ...
        for (var i in ani_pms) {
            var pm = ani_pms[i];
            var clr = pm.GetFieldValueByIndex(6);
            var rgb = HexToRGB(clr);
            Highlight(pm, false, 0, rgb);
        }
        ani_pms = [];
    }
    function FeederAni_Calculate() {
        ...
        if (start_ufid == null || end_ufids.length == 0)
            return;
        ...
        $.get(url, null, function (returnval) {
            var rt = returnval;
            if (rt == null)
                return;
            ...
            var pms = [];
            pms = pms.concat(LineData.GetPlacemarks());
            ...
            else if (dir == 0) // 無向, 先播下游, 再播上游
            {
                ...
                for (var i = 0; i < ani_pms.length; i++) {
                    if (ani_pms[i].GetFieldValueByIndex(2) == start_ufid) {
                        // 截斷上游
        ...
        //// 統計結果
        //var msg = "共 " + result.dvCount.toString() + " 具, 加總為 " + result.attSum.toString() + ".";
        //document.getElementById("StatsResult").textContent = msg;
    ...
    function CollectAniPlacemarks(dv, pms) {
        for (var i in pms) {
            if (pms[i].GetFieldValueByIndex(2) == dv.ufid) {
                ani_pms.push(pms[i]);
                //break;
            }
        }
    ...
    function PlayAni() {
        ...
        ani_interval = setInterval(function () {
            if (ani_playidx != ani_pms.length) {
                ...
                Highlight(pm, false, ms / 1000, [1, 1, 0]);
            }
            else
    ...
    function Highlight(pm, restore, sec, color) {
        ...
        pm.Highlight(color, restore, sec);
    }
    ...
    var div11 = this.BodyCell.appendObject("table", { id: "divDrawGraphCHKBOX" }, { position: "absolute", left: "0", bottom: "0", width: "140px", height: "60px" });
    ...
    td1.appendObject("label", null, { "letter-spacing": "0.125em", position: "absolute", left: "14px", top: "9px" }).appendText("饋線範圍");
    ...
    td2.appendObject("img", { src: "../images/ToolWindow/Topology/StartNode.svg" }, { position: "absolute", left: "9px", top: "5px" });
    td2.appendObject("label", null, { position: "absolute", left: "35px", top: "6px", "letter-spacing": "0.125em" }).appendText("起始節點");
    td2.appendObject("input", { id: "iptFeederLineStartNode", class: "NodeFeederInput", type: "text" }, { top: "0" });
    ...
    td2.appendObject("label", null, { position: "absolute", left: "35px", top: "50px", "letter-spacing": "0.125em" }).appendText("終止節點");
    ...
    td3.appendObject("label", null, { position: "absolute", left: "0", top: "0px", letterSpacing: "0.125em" }).appendText("屬性計算");
    td3.appendObject("label", null, { position: "absolute", left: "20px", top: "20px" }).appendText("設備類別");
    var sel71 = td3.appendObject("select", { id: "selEquipClass" }, { position: "absolute", left: "83px", top: "22px", width: "158px", height: "26px" });
    ...
    td3.appendObject("label", null, { position: "absolute", left: "20px", top: "50px" }).appendText("設備屬性");
    var sel72 = td3.appendObject("select", { id: "selEquipProperty" }, { position: "absolute", left: "83px", top: "55px", width: "158px", height: "26px" });
    ...
    td4.appendObject("label", null, { "letter-spacing": "0.125em", position: "absolute", left: "14px", bottom: "6px" }).appendText("動畫效果間隔(毫秒)");
