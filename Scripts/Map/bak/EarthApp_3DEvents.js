    ...
    PickPlacemark: function (x, y) {
		var feature = null;
			for (var i = x - 2; i < x + 2; i++)
			{
				...
				{
					feature = LineData.HitTestFeature(i, j);
					if (feature)
						break;
				}
				if (feature)
    ...
    CheckChangeInfo: function () {
        ...
        xmlHttp.onload = function (e) {
			...
			else
            {
				...
				$.post("../GisMap/GetPowerRate", { JsonTmp_DVS: JSON.stringify(newdvs) }, function (res) {
					if (res.ProcessFlag) {
						var objRes = JSON.parse(res.ProcessMessage);
						$("#divPowerShow").html("受影響區間負載量：" + parseFloat(objRes.PowerFailures).toFixed(2) + "千瓦");
						$("#divPowerShow").show();
					}
				});
			}
            ...
            for (var i = 0; i < tmp_dvs.length; i++) {
				...
				{
					var pm = pms[j];
					if ((IsPlacemark(pm) && pm.GetMeshes() == null) || (!IsPlacemark(pm) && pm.parent.VecBuf == null)) // 有 pm 未載入完成
					{
			...
			for (var i in eventMarkers.fdr_labels)
			{
				...
				var marker = new SuperGIS.Marker(earth_, loc, "", "../images/" + state + ".png",{FixedSize : true});
